defimpl Canada.Can, for: TrueBG.Accounts.User do
  ...
  def can?(%TrueBG.Accounts.User{id: user_id}, :create_business_concept, %TrueBG.Taxonomies.DataDomain{id: resource_id})  do
    ...
    permissions = TrueBG.BusinessConcepts.BusinessConcept.get_permissions()
  ...
  def can?(%TrueBG.Accounts.User{}, _action, TrueBG.BusinessConcepts.BusinessConcept) do  #when action in [:admin, :watch, :creaBusinte, :publish] do
    true
  end

  def can?(%TrueBG.Accounts.User{id: user_id}, :update, %TrueBG.BusinessConcepts.BusinessConcept{status: status} = business_object) do
    resource_id = business_object.data_domain_id
    role = TrueBG.Permissions.get_role_in_resource(%{user_id: user_id, data_domain_id: resource_id})
    role_name = String.to_atom(role.name)
    permissions = TrueBG.BusinessConcepts.BusinessConcept.get_permissions()
    Enum.member?(permissions[role_name], :update) &&
      status == Atom.to_string(TrueBG.BusinessConcepts.BusinessConcept.draft)
  end

  def can?(%TrueBG.Accounts.User{id: user_id}, :send_for_approval, %TrueBG.BusinessConcepts.BusinessConcept{status: status} = business_object) do
    resource_id = business_object.data_domain_id
    role = TrueBG.Permissions.get_role_in_resource(%{user_id: user_id, data_domain_id: resource_id})
    role_name = String.to_atom(role.name)
    permissions = TrueBG.BusinessConcepts.BusinessConcept.get_permissions()
  ...
  def can?(%TrueBG.Accounts.User{id: user_id}, :reject, %TrueBG.BusinessConcepts.BusinessConcept{status: status} = business_object) do
    resource_id = business_object.data_domain_id
    role = TrueBG.Permissions.get_role_in_resource(%{user_id: user_id, data_domain_id: resource_id})
    role_name = String.to_atom(role.name)
    permissions = TrueBG.BusinessConcepts.BusinessConcept.get_permissions()
    Enum.member?(permissions[role_name], :reject) &&
      status == Atom.to_string(TrueBG.BusinessConcepts.BusinessConcept.pending_approval)
  end

  def can?(%TrueBG.Accounts.User{id: user_id}, :publish, %TrueBG.BusinessConcepts.BusinessConcept{status: status} = business_object) do
    resource_id = business_object.data_domain_id
    role = TrueBG.Permissions.get_role_in_resource(%{user_id: user_id, data_domain_id: resource_id})
    role_name = String.to_atom(role.name)
    permissions = TrueBG.BusinessConcepts.BusinessConcept.get_permissions()
