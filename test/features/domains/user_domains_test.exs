defmodule TdBg.UserDomainsTest do
  ...
  defwhen ~r/^user "(?<user_name>[^"]+)" lists the domains where he has a given permission$/,
          %{user_name: user_name},
          state do
  ...
  defand ~r/^if result "(?<result>[^"]+)" the retrieved list of domains should not be empty$/,
         %{result: result},
         state do
  ...
  defand ~r/^the following domains should be in the list:$/, %{table: table}, state do
    ...
    assert Enum.all?(
             Enum.map(table, & &1.domain),
             fn x -> Enum.member?(Enum.map(data, & &1["domain_name"]), x) end
           )
  end
  ...
  defand ~r/^"(?<group_name>[^"]+)" has a role "(?<role_name>[^"]+)" in Domain "(?<domain_name>[^"]+)"$/,
         %{group_name: group_name, role_name: role_name, domain_name: domain_name},
         state do
    # Your implementation here
    domain_info = get_domain_by_name(state[:token_admin], domain_name)
