defmodule TdBg.RolesAdminTest do
  ...
  defp data_domain_users_roles(token, attrs) do
    ...
    %HTTPoison.Response{status_code: status_code, body: resp} =
      HTTPoison.get!(data_domain_data_domain_url(TdBgWeb.Endpoint, :users_roles, attrs.id), headers, [])
  ...
  defp domain_group_users_roles(token, attrs) do
    headers = get_header(token)
    %HTTPoison.Response{status_code: status_code, body: resp} =
      HTTPoison.get!(domain_group_domain_group_url(TdBgWeb.Endpoint, :users_roles, attrs.id), headers, [])
    {:ok, status_code, resp |> JSON.decode!}
  end
  ...
  defp user_data_domain_role(token, attrs) do
    headers = get_header(token)
    %HTTPoison.Response{status_code: status_code, body: resp} =
      HTTPoison.get!(user_data_domain_role_url(TdBgWeb.Endpoint, :user_data_domain_role, attrs.user_id, attrs.data_domain_id), headers, [])
    {:ok, status_code, resp |> JSON.decode!}
  end
