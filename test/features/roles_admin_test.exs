defmodule TrueBG.RolesAdminTest do
  ...
  defand ~r/^following users exist with the indicated role in Domain Group "(?<domain_group_name>[^"]+)"$/,
    ...
    Enum.map(table, fn(x) ->
        user_name = x[:user]
        role_name = x[:role]
        {:ok, 201, %{"data" => %{"id" => principal_id}}} = user_create(state[:token_admin], %{user_name: user_name, password: user_name})
  ...
  defand ~r/^an existing user "(?<user_name>[^"]+)"$/, %{user_name: user_name}, state do
    # Your implementation here
    user = get_user_by_name(state[:token_admin], user_name)
    unless user do
      user_create(state[:token_admin], %{user_name: user_name, password: user_name})
    end
    {:ok, state}
  end
  ...
  defwhen ~r/^"(?<user_name>[^"]+)" grants (?<role_name>[^"]+) role to user "(?<principal_name>[^"]+)" in Domain Group "(?<resource_name>[^"]+)"$/,
    ...
    {_, _status_code, json_resp} = session_create(user_name, user_name)
    ...
    {_, status_code, json_resp} = acl_entry_create(json_resp["token"] , acl_entry_params)
  ...
  defand ~r/^if result "(?<actual_result>[^"]+)" is "(?<expected_result>[^"]+)", the user "(?<user_name>[^"]+)" has "(?<role_name>[^"]+)" role in Domain Group "(?<domain_group_name>[^"]+)"$/,
    ...
    if actual_result == expected_result do
      ...
      acl_entry_params = %{user_id: user_info["id"], domain_group_id: domain_group_info["id"]}
  ...
  defand ~r/^if result "(?<actual_result>[^"]+)" is "(?<expected_result>[^"]+)", the user "(?<user_name>[^"]+)" has "(?<role_name>[^"]+)" role in Data Domain "(?<data_domain_name>[^"]+)"$/,
    ...
    if actual_result == expected_result do
      ...
      user_info = get_user_by_name(state[:token_admin], user_name)
      acl_entry_params = %{user_id: user_info["id"], data_domain_id: data_domain_info["id"]}
