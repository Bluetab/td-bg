defmodule TdBg.CacheConfigTest do
  use ExUnit.Case
  ...
  describe "td-cache configuration from environment variables" do
    test "reads REDIS_AUDIT_STREAM_MAXLEN from environment" do
      System.put_env("REDIS_AUDIT_STREAM_MAXLEN", "180")
    ...
    test "reads REDIS_STREAM_MAXLEN from environment" do
      ...
      assert Keyword.get(event_stream_config, :maxlen) == "220"
    ...
    test "configuration preserves business concept stream consumer" do
      System.put_env("REDIS_STREAM_MAXLEN", "450")
      ...
      assert Keyword.get(event_stream_config, :maxlen) == "450"
      assert Keyword.get(event_stream_config, :consumer_group) == "bg"

      streams = Keyword.get(event_stream_config, :streams)
      assert length(streams) == 2

      business_concept_stream =
        Enum.find(streams, &(Keyword.get(&1, :key) == "business_concept:events"))

      assert Keyword.get(business_concept_stream, :consumer) == TdBg.Cache.ConceptLoader
