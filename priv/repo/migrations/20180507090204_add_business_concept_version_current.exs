  ...
  def up do
    ...
    query =
      from(v in "business_concept_versions",
        select: %{business_concept_id: v.business_concept_id, version: max(v.version)},
        group_by: v.business_concept_id
      )

    current_versions =
      from(v in "business_concept_versions",
        join: s in subquery(query),
        on: s.business_concept_id == v.business_concept_id,
        select: v.id,
    ...
    case current_versions do
      ...
      _ ->
        from(v in "business_concept_versions",
          ...
          where: v.id in ^current_versions
        )
        |> Repo.update_all([])

        from(v in "business_concept_versions",
          update: [set: [current: false]],
          where: v.id not in ^current_versions
        )
        |> Repo.update_all([])
    end
